...
main()
{
	...
	if ! getent group "${PRIMARY_GID}" >/dev/null 2>&1; then
		curl -H 'Cache-Control: no-cache' -fsSL https://github.com/humblebeeai/script-common/raw/main/src/account/unix/linux/create-group.sh | \
	...
	if ! id "${USERNAME}" >/dev/null 2>&1; then
		...
		curl -H 'Cache-Control: no-cache' -fsSL https://github.com/humblebeeai/script-common/raw/main/src/account/unix/linux/create-user.sh | \
		...
		curl -H 'Cache-Control: no-cache' -fsSL https://github.com/humblebeeai/script-common/raw/main/src/account/unix/linux/change-password.sh | \
	...
	if getent group docker >/dev/null 2>&1 && ! id -nG "${USERNAME}" | grep -qw docker; then
		curl -H 'Cache-Control: no-cache' -fsSL https://github.com/humblebeeai/script-common/raw/main/src/account/unix/linux/add-user-group.sh | \
	...
	${_SUDO} su - "${USERNAME}" -c "curl -H 'Cache-Control: no-cache' -fsSL https://github.com/humblebeeai/script-common/raw/main/src/setup/unix/setup-user-env.sh | bash ${_arg_all_runtimes}" || {
